...
def rnn_model(X, Xtest, y, yTest):
    ...
    for i in range(len(y)):
        DStrain.addSample(X[i],y[i])
    print 'start training rnn'
    trainer = BackpropTrainer(n, DStrain, verbose = True, learningrate=0.01)
    trainer.trainUntilConvergence(maxEpochs=120)
    DStest = SupervisedDataSet(Xtest.shape[1],yTest.shape[1])
    for i in range(len(yTest)):
        DStest.addSample(Xtest[i],yTest[i])
    prediction = n.activate(DStest)
    plt.figure()
    plt.plot(np.arange(0,len(yTest)), prediction, 'ro--', label='predict number')
    ...
    error = model_evaluate(prediction, y)
    return error
